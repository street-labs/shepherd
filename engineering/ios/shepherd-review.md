...
- `RelayClient` (shared, reused):
  ...
  - relay URL resolution: in-app configured list (Settings, `FR-sri-relay-settings`) → `NOSTR_RELAYS` env → `~/.config/nostr/relays.txt` → default public fallback. The in-app list is persisted in `UserDefaults` (relay URLs are not secrets; the native facility for non-secret preferences) under key `shepherd.relays` and takes highest precedence, mirroring the identity feature's in-app-Keychain-first precedence. Added to the shared `RelayClient` (used by both macOS and iOS), so both surfaces honor an in-app-saved list when present; on macOS the out-of-band env/file sources continue to apply when no in-app list is saved.
- `SettingsFeature` (shared module `AppFeature`, iOS-surfaced): `State { relays: [String], useDefaults: Bool, newRelay: String, error: String? }`; actions `binding`, `addRelayTapped` (validates `wss://`/`ws://` URL, `AC-sri-relay-invalid`), `removeRelay(String)`, `defaultsToggled`, `saveTapped` (persists via `RelayClient.saveConfiguredRelays`, emits `delegate.settingsSaved`). `AppFeature` presents it as a sheet (`settingsRequested` / `settings(PresentationAction<SettingsFeature.Action>)`); `iOSAppView` opens it from a toolbar gear and from the empty state.
...
- `SettingsFeature` (shared `AppFeature` module, new) — the Settings (Relays) sheet reducer: edit/validate the relay list, "use defaults" toggle, persist via `RelayClient`. Surfaced on iOS only (`iOSAppView`); macOS keeps its out-of-band sources (`FR-sri-relay-settings`).
...
3. **Settings (relays)** — `SettingsFeature` (new, shared `AppFeature` module) + UserDefaults-backed configured-relay source in `RelayClient.resolveRelays` (highest precedence), surfaced from `iOSAppView` (`FR-sri-relay-settings`; Open Question 3 resolved).
...
| `FR-sri-relay-settings` | engineering/apps/macos/Sources/Dependencies/RelayClient.swift; engineering/apps/macos/Sources/AppFeature/SettingsFeature.swift; engineering/apps/ios/ShepherdiOSApp/iOSAppView.swift | implemented |
...
3. **Relay default set** — resolved: reuse the macOS default set (`wss://relay.damus.io`, `wss://nos.lol`, `wss://relay.nostr.band`). The settings UI is a Settings sheet (`SettingsFeature` in the shared `AppFeature` module, surfaced from `iOSAppView`); see `FR-sri-relay-settings` and `../../design/ios/shepherd-review.md` Settings (Relays).
