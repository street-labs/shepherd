...
export function RenderedDiffFallbackBanner({ onSwitchToRawDiff }: RenderedDiffFallbackBannerProps) {
  ...
  return (
    <div className="mx-4 my-2 px-4 py-3 rounded border border-warning-border bg-warning-bg flex items-center justify-between">
      <div className="flex items-center gap-2 text-sm text-warning-text">
        ...
        <span>
          ...
          <button onClick={onSwitchToRawDiff} className="underline font-medium hover:opacity-80">
            Switch to Raw Diff
          </button>
        </span>
      </div>
      <button
        ...
        className="text-warning-text hover:opacity-80 p-1"
        aria-label="Dismiss"
      >
