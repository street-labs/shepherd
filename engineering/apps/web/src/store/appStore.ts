...
import { buildMultiFilePrompt, buildMultiFileDiffPrompt, buildRenderedPrompt, buildRenderedDiffPrompt } from '@/lib/promptBuilder';
...
function computeDiffCommentOrder(comments: Record<string, DiffComment>, fileId?: string | null): string[] {
  return Object.values(comments)
    .filter((c) => !fileId || c.fileId === fileId)
    .sort((a, b) => {
...
/** Auto-generate the prompt for diff mode. Uses multi-file aggregation across all files. */
function autoGenerateDiffPrompt(state: FullState): string | null {
  const { files, fileOrder, diffComments, preamble } = state;
  if (fileOrder.length === 0) return null;
  // Use multi-file diff prompt which aggregates all files' diff comments via stored contextSnippet
  return buildMultiFileDiffPrompt(files, fileOrder, diffComments, preamble);
}
...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  removeFile: (fileId) => {
    ...
    // Remove all comments for this file (file-mode and diff-mode)
    ...
    const newDiffComments: Record<string, DiffComment> = {};
    for (const [cid, comment] of Object.entries(get().diffComments)) {
      if (comment.fileId !== fileId) {
        newDiffComments[cid] = comment;
      }
    }
  ...
  setActiveFile: (fileId) => {
    ...
    // Recompute diff comment order for the target file (diff comments persist across tab switches)
    const diffCommentOrder = computeDiffCommentOrder(get().diffComments, fileId);

    set({
      ...initialDiffState,
      ...initialRenderedState,
      // Preserve diff comments across tab switches (they have fileId for grouping)
      diffComments: get().diffComments,
      diffCommentOrder,
      activeFileId: fileId,
      file: targetFile,
      commentOrder,
      ...
      renderMode: 'raw', // Default to raw — rendered markdown experience needs improvement
      isMarkdownFile: isMd,
  ...
  addDiffComment: (startIndex, endIndex, text) => {
    const { diffLines, activeFileId } = get();
    if (!diffLines || !activeFileId) return;

    const startLine = diffLines[startIndex];
    const endLine = diffLines[endIndex];
    if (!startLine || !endLine) return;

    // Pre-compute the diff context snippet so prompt can be built without live diffLines
    const CONTEXT_LINES = 2;
    const snippetStart = Math.max(0, startIndex - CONTEXT_LINES);
    const snippetEnd = Math.min(diffLines.length - 1, endIndex + CONTEXT_LINES);
    ...
    const diffCommentOrder = computeDiffCommentOrder(diffComments, activeFileId);
  ...
  deleteDiffComment: (commentId) => {
    ...
    const diffCommentOrder = computeDiffCommentOrder(rest, get().activeFileId);
  ...
  toggleReviewContextSidebarCollapsed: () => {
    set({ isReviewContextSidebarCollapsed: !get().isReviewContextSidebarCollapsed });
  },
