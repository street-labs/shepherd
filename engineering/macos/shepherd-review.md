...
| `engineering/apps/macos/Sources/Dependencies/BunkerClient.swift` | **NEW** | NIP-46 bunker control channel over `RelayClient`: generates an ephemeral session keypair, sends an encrypted (NIP-04) kind `24133` `connect` request (with `secret` when present), then `get_pubkey` and `sign_event` requests; yields the bunker's signed event on `sign_event`. Reuses the relay named in the bunker URI. Implements `FR-srm-bunker-connect`, the bunker half of `FR-srm-event-sign`, `FR-srm-bunker-sign-failure`, `FR-sr-bunker-signing`. |
| `engineering/apps/macos/Sources/Dependencies/NIP04Crypto.swift` | **NEW** | Minimal NIP-04 payload encryption/decrypt: ECDH shared secret via `P256K` (already a dependency) + AES-256-CBC via `CryptoKit.AES.GCM`-style CBC using `CryptoKit`. No new package dependency. Used only by `BunkerClient` for the kind `24133` control channel. |
...
1. Environment variable `SHEPHERD_BUNKER` — a `bunker://<remote-signer-pubkey>?relay=<wss-url>[&secret=<token>]` URI.
...
When the loaded identity is a bunker connection, a `BunkerClient` `@Dependency` runs the NIP-46 handshake over the relay named in the bunker URI, reusing the in-process `RelayClient` transport (`FR-sr-relay-client`) — no new WebSocket code, no external process. The flow:

1. Generate an ephemeral secp256k1 session keypair (in-process via `P256K`); the session pubkey is the NIP-46 client identity for this review window.
2. Send a NIP-46 `connect` request: a kind `24133` event, `p`-tagged to the bunker's pubkey, with `content` = NIP-04-encrypted JSON `{"method": "connect", "params": ["<session-pubkey>", "<secret?>"]}` (the `secret` from the URI is included when present). `NIP04Crypto` (new, `Sources/Dependencies/NIP04Crypto.swift`) provides the ECDH shared secret (`P256K`, already a dependency) + AES-256-CBC (`CryptoKit`) encrypt/decrypt — no new Swift package.
3. Await the bunker's kind `24133` response (encrypted to the session pubkey). On `connect` success, send a `get_pubkey` request and set the reviewer's public key to the returned pubkey; flip the connection state to `.connected`. On refusal (e.g. bad `secret`) or timeout, set `.failed` with a one-line cause; publishing is unavailable but read-only review and local commenting remain available.
...
1. `NIP04Crypto` (`Sources/Dependencies/NIP04Crypto.swift`): ECDH shared secret via `P256K` + AES-256-CBC via `CryptoKit`; `encrypt(_:toPeer:with:)` / `decrypt(_:fromPeer:with:)`. Pure, unit-tested standalone. No new package dependency.
2. `BunkerClient` (`Sources/Dependencies/BunkerClient.swift`): parse `bunker://` URIs; generate an ephemeral session keypair; over `RelayClient` to the bunker relay, send NIP-04-encrypted kind `24133` `connect` (with `secret`), then `get_pubkey`, then `sign_event` per reply. Expose `connect() async -> Bool`, `reviewerPubkey() -> String?`, `signEvent(_:) async -> NostrEvent?`, and a connection-state stream for the indicator. `testValue` is an injectable mock bunker.
...
8. Tests: `BunkerClientTests` against an in-process mock bunker (a fake `RelayClient` echoing kind `24133` responses); `NIP04CryptoTests` round-trip; `IdentityClientTests` for `bunker://` parsing + precedence + malformed URI; extend `NostrSignerTests` for the async bunker path.
...
| `FR-sr-bunker-signing` | engineering/apps/macos/Sources/Dependencies/BunkerClient.swift; engineering/apps/macos/Sources/Dependencies/NostrSigner.swift; engineering/apps/macos/Sources/AppFeature/AppFeature.swift | planned |
...
| `FR-srm-bunker-connect` | engineering/apps/macos/Sources/Dependencies/BunkerClient.swift; engineering/apps/macos/Sources/Dependencies/NIP04Crypto.swift; engineering/apps/macos/Sources/AppFeature/AppFeature.swift | planned |
| `FR-srm-bunker-sign-failure` | engineering/apps/macos/Sources/Dependencies/BunkerClient.swift; engineering/apps/macos/Sources/AppFeature/AppFeature.swift; engineering/apps/macos/Sources/CommentFeature/InlineCommentEditorView.swift | planned |
...
| `FR-srm-bunker-connect` | Patch-thread reply publishing section (BunkerClient NIP-46 connect/get_pubkey over RelayClient; NIP04Crypto) |
