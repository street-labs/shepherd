...
The iteration loop is replaced with a batch-open + wait-for-done model: all files open at once, the user reviews freely in the CRPG with context and review feedback visible in the tool UI, clicks "Done" to generate a unified multi-file prompt, and the agent presents an interactive prompt (`AskUserQuestion`) after launching the CRPG, and reads `~/.shepherd/sessions/<session-id>/prompt-output.md` when the user selects 'Added comments'. Session state is minimal -- just the file list, counts, the session ID, and the prompt output. The session ID is generated by `shepherd-launch.sh` and passed through the URL to the CRPG (`FR-sc-session-id`).
...
| Context handoff | JSON file at `~/.shepherd/sessions/<session-id>/review-context.json` | The agent writes structured context data (overall + per-file, each with neutral + review) to a session-scoped JSON file before launching. The CRPG reads it on load via `GET /api/review-context?session=<id>`. This avoids URL length limits and is consistent with the session-scoped `prompt-output.md` file-based handoff pattern (`FR-sc-session-scoped-output`). |
...
| Prompt return | Interactive prompt (`AskUserQuestion`) | After opening the browser, the agent presents an interactive prompt with three options: 'Added comments' (reads `~/.shepherd/sessions/<session-id>/prompt-output.md`), 'Reviewed, no comments' (zero-feedback summary), 'Cancel' (end session). Simpler and more reliable than file-watcher polling -- no timeout management, no race conditions. |
...
7. **Step 6: Write context file and display brief summary** -- Write the structured context data to `~/.shepherd/sessions/<session-id>/review-context.json` (`FR-sr-context-handoff`, `FR-sc-session-scoped-output`). The session ID is obtained from the output of `shepherd-launch.sh` (the launch script generates it and prints `Session: <id>` to stdout). Display a brief summary in the conversation: session ID, scope label, file count, and exclusion count. The detailed changeset overview, per-file context, and review feedback are NOT displayed in the conversation -- they are in the context file for the CRPG to display.
8. **Step 7: Auto-open all files** -- Immediately invoke `shepherd-launch.sh` with all file paths (no confirmation prompt, `AC-sr-auto-open`). The launch script generates a session ID and includes it in the URL (`?session=<id>&file=...`). Clean up stale `~/.shepherd/sessions/<session-id>/prompt-output.md`, present an interactive prompt (`AskUserQuestion`) with three options: 'Added comments', 'Reviewed, no comments', 'Cancel'. Read prompt output from `~/.shepherd/sessions/<session-id>/prompt-output.md` if user selects 'Added comments'. Clean up the session directory after reading.
...
After reading diffs for all reviewable files, the agent generates structured context and writes it to `~/.shepherd/sessions/<session-id>/review-context.json` before launching the CRPG. The CRPG reads this file on load via the `GET /api/review-context?session=<id>` Vite plugin endpoint (see `../engineering/code-review-prompt.md`). The session ID ensures concurrent `/shepherd-review` invocations do not overwrite each other's context data (`FR-sc-session-scoped-output`).
...
The agent writes the context data to `~/.shepherd/sessions/<session-id>/review-context.json` using the `Write` tool. The JSON structure:
...
2. Agent invokes `shepherd-launch.sh` with all file paths. The launch script generates a session ID and prints `Session: <id>` to stdout. The agent captures this session ID.
3. Agent writes `~/.shepherd/sessions/<session-id>/review-context.json` using the `Write` tool (creates the session directory if needed).
4. Agent displays the brief conversation summary (session ID, scope, file count, exclusion count).
5. CRPG loads, reads the context file via `GET /api/review-context?session=<id>`, and displays it in the ReviewContextPanel.

The context file is written once per `/shepherd-review` invocation to a session-scoped directory. Each concurrent invocation writes to its own session directory, preventing conflicts. The CRPG reads it once on load and holds the data in memory.
...
The session ID line identifies this review session (`FR-sc-session-id`). The exclusion line appears only if `<M>` is greater than zero. The detailed context is in the session-scoped JSON file for the CRPG to display.
...
| `session_id` | string | The session ID derived from the project directory by `shepherd-launch.sh` and captured from its stdout (e.g., `shepherd-1`, `my-project`). Used to scope all session files under `~/.shepherd/sessions/<session-id>/` (`FR-sc-session-id`). |
| `prompt_output` | string | The content of `~/.shepherd/sessions/<session-id>/prompt-output.md` returned by the CRPG when the user clicks "Done" |
...
2. Agent immediately invokes `shepherd-launch.sh` with all file paths as absolute paths (`AC-sr-auto-open`). No "go"/"quit" prompt. The launch script generates a session ID and prints `Session: <id>` to stdout. The agent captures this as `session_id`.
3. Agent writes `~/.shepherd/sessions/<session-id>/review-context.json` to disk (`FR-sr-context-handoff`, `FR-sc-session-scoped-output`).
4. Agent displays a brief conversation summary (session ID, scope, file count, exclusion count).
5. Agent cleans up stale `~/.shepherd/sessions/<session-id>/prompt-output.md` (deletes if exists).
6. Agent presents `AskUserQuestion` with three options: "Added comments", "Reviewed, no comments", "Cancel".
7. Based on user's selection: "Added comments" → read `~/.shepherd/sessions/<session-id>/prompt-output.md`; "Reviewed, no comments" → proceed with zero comments; "Cancel" → end immediately. In all cases, clean up the session directory after use (`FR-sc-session-cleanup`).
...
2. **URL construction**: Construct a URL with the session ID and multiple `file` query parameters, one per file. Each path is URL-encoded independently. Example: `http://localhost:<port>?session=<id>&file=%2Fpath%2Fto%2Ffile1.ts&file=%2Fpath%2Fto%2Ffile2.ts&file=%2Fpath%2Fto%2Ffile3.ts`
...
5. **Summary output**: Updated to report the session ID and number of files opened (e.g., `Session: my-project\nOpened CRPG at http://localhost:54321 — loaded 5 files (reusing server)`).
...
1. **Cleans up stale output**: Deletes `~/.shepherd/sessions/<session-id>/prompt-output.md` if it exists from a previous invocation with the same session ID (unlikely but defensive).
...
3. **If 'Added comments'**: Reads `~/.shepherd/sessions/<session-id>/prompt-output.md` and stores the content as the unified multi-file prompt. Cleans up the session directory after reading (`FR-sc-session-cleanup`).

The CRPG writes to `~/.shepherd/sessions/<session-id>/prompt-output.md` (via `POST /api/prompt-output?session=<id>`) when the user clicks "Done" -- the session ID in the URL ensures each concurrent session writes to its own scoped directory. The interactive prompt replaces the file-watcher polling mechanism with a simpler, more reliable approach.
...
[Invoke shepherd-launch.sh first to get the session ID from its stdout.
 Write ~/.shepherd/sessions/<session-id>/review-context.json with the
 structured context data. Display brief conversation summary: session ID,
 scope label, file count, exclusion count. Do NOT display the detailed
 changeset overview, per-file context, or review feedback in the
 conversation.]

### Step 7: Auto-open all files
[shepherd-launch.sh was already invoked in step 6. It generates the
 session ID, opens the browser with ?session=<id>&file=... URL params.
 No confirmation prompt -- the CRPG opens automatically.
 Delete stale ~/.shepherd/sessions/<session-id>/prompt-output.md,
 present AskUserQuestion with three options:
 'Added comments' (read ~/.shepherd/sessions/<session-id>/prompt-output.md),
 'Reviewed, no comments' (proceed with zero comments),
 'Cancel' (end session).
 Clean up session directory after reading.]
...
shepherd/                                    (project root)
  ...
  ~/.shepherd/
    sessions/<session-id>/
      ...
      prompt-output.md                       RUNTIME -- written by the CRPG when user clicks Done, read by the agent
    servers/<project-hash>.lock              RUNTIME -- per-project lock file recording dynamic port (written by shepherd-launch.sh)
```
...
- Write the context data to `~/.shepherd/sessions/<session-id>/review-context.json` (`FR-sr-context-handoff`, `FR-sc-session-scoped-output`).
...
- Clean up stale prompt output, present `AskUserQuestion` with three options ('Added comments', 'Reviewed, no comments', 'Cancel'), read `~/.shepherd/sessions/<session-id>/prompt-output.md` if user selects 'Added comments'. Clean up session directory after reading (`FR-sc-session-cleanup`).
...
5. Context file `~/.shepherd/sessions/<session-id>/review-context.json` is written with valid JSON containing overall and per-file context with both neutral and review fields.
...
11. The prompt output is returned via `~/.shepherd/sessions/<session-id>/prompt-output.md`.
...
| `FR-sr-context-handoff` | Context Data Handoff section; `~/.shepherd/sessions/<session-id>/review-context.json` file format; Command file Step 6 (writes context file before launch); CRPG reads via `GET /api/review-context?session=<id>` |
...
| `AC-sr-context-in-crpg` | Context Data Handoff section (structured JSON written by agent to `~/.shepherd/sessions/<session-id>/review-context.json`, read by CRPG via `GET /api/review-context?session=<id>`, displayed in ReviewContextPanel) |
...
| `AC-sr-unified-prompt` | Multi-File Launch section -- CRPG generates one multi-file prompt via session-scoped `~/.shepherd/sessions/<session-id>/prompt-output.md` |
