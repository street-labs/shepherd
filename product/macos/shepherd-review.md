...
#### `FR-srm-relay-settings` -- Configure Nostr relays in-app (parity with iOS)
The native macOS app offers the same in-app Settings (Relays) surface as iOS (`FR-sri-relay-settings`): add/remove relay URLs with inline `wss://`/`ws://` validation, a "use defaults" toggle, and Save, reachable from the app menu (⌘,). The saved list persists in-app and takes highest precedence in `RelayClient.resolveRelays()` — above `NOSTR_RELAYS`, `~/.config/nostr/relays.txt`, and the default public set — mirroring the identity feature's in-app-first precedence (`FR-id-no-silent-override`). The out-of-band env/file sources continue to apply when no in-app list is saved.
...
#### `FR-srm-pr-approve` -- Approve a PR/patch from the review surface
During a patch or PR review with an identity loaded, the reviewer can approve the change under review with one action. Approving publishes a signed kind:1 approval note to the patch/PR thread -- tagged with the PR/patch event as `e`, the repository `a` tag when present, the tip commit `c` tag when known, and a `t` tag of `approval` -- the convention downstream merge automation watches for. The approval control shows its progress and outcome (publishing, approved, or a retryable failure naming the cause) and is disabled when no identity is loaded. Approving does not close or alter the local review session.
...
When the reviewer submits a valid reference, the app fetches the patch event in-process using `FR-sr-relay-client`: it opens a NIP-01 subscription whose filter is the **event id only** (`{"ids": ["<id>"]}`), with **no `kinds` filter**, across the configured relays. Fetching by `ids` alone (rather than `ids` + `kinds`) is deliberate: it lets the app receive the event whatever its kind, then reject non-patch kinds explicitly with the wrong-kind message — so a reviewer who pastes a kind:1 note id or a kind:1621 issue id by mistake gets a precise error instead of an opaque "not found" (the relay would otherwise filter a non-1617 id out and return nothing). When the decoded `nevent1` carries relay hints, those relays are preferred; otherwise the standard relay resolution (`NOSTR_RELAYS`, `~/.config/nostr/relays.txt`, default public relays) is used.
...
2. **Attach patch metadata.** The app builds a patch metadata record from the event — full and short event id, author (event pubkey, resolved to a display name via the roster when available), commit message (the first line of the event content, i.e. the `git format-patch` subject before the diff body), parent commit short hash (from a `parent-commit` tag, if present), and repo coordinate (the `a` tag, when present) — and sets it on the session's patch metadata. This activates the patch metadata section, the live patch-thread reply subscription (`FR-sr-patch-replies-live`), and the reply-publishing path (`FR-srm-comment-publish-on-submit`).

   **Patch status** is shown as `open` unconditionally in v1. NIP-34 does not carry status on the patch event itself — status is conveyed by separate kind `1630`–`1633` status events that reference the patch via an `e` tag, the most recent of which (by `created_at`) from the patch author or a repo maintainer is valid. The in-app v1 path does not fetch status events; it renders `open` and leaves status-event resolution as a roadmap fast-follow (`roadmap/patch-watcher.md`). (The shared `FR-sr-patch-metadata-display` currently reads a `status` tag; per NIP-34 no such tag exists on patch events. See Open Questions.)
...
- [ ] **Relays configured in-app** `AC-srm-relay-settings`: Given the reviewer opens Settings (⌘,) and saves a custom relay list, then the list persists across launches, `RelayClient.resolveRelays()` returns exactly the saved list (above env/file sources), and subscriptions/publishes use it. Given "use defaults" is active, then the default public set is used and the out-of-band env/file sources apply when no in-app list was saved. Given an invalid URL is entered, then an inline validation message shows and nothing is saved. (Parity with `AC-sri-relay-*`; the shared `SettingsFeatureTests` cover both.)
...
- [ ] **Open Patch from empty state** `AC-srm-patch-open-happy`: Given the native app is in its empty state (no files loaded), when the reviewer opens the "Open Patch…" affordance, pastes a 64-character hex event id for a valid NIP-34 patch event (kind `1617`) whose content is a unified diff, and submits, then the app fetches the event in-process from the configured relays, splits the diff into one tab per changed file named by file path, attaches patch metadata (author, message, parent, status `open`, repo coordinate), and enters the multi-file review layout with the patch metadata section, live patch-thread replies, and reply-publishing path all active — without invoking `/shepherd-review` or any shell process and without requiring a local git repository.

- [ ] **nevent reference accepted** `AC-srm-patch-open-nevent`: Given the reviewer pastes a `nevent1…` reference that encodes a patch event id, when the app decodes it, then it fetches the referenced event from the relays encoded in the reference (preferred over the default relay list) and proceeds as in `AC-srm-patch-open-happy`.

- [ ] **Invalid reference rejected inline** `AC-srm-patch-open-invalid-id`: Given the reviewer enters text that is neither a 64-character hex event id nor a `nevent1` reference, when they submit, then the dialog shows "Enter a 64-character hex event id or a nevent1 reference", no fetch is attempted, and the dialog stays open for correction.
...
- [ ] **Non-patch event rejected** `AC-srm-patch-open-wrong-kind`: Given the reviewer pastes the id of a kind:1 text note (or a kind:1621 issue, or any non-`1617` event) that exists on the relays, when the app fetches by id and validates the kind, then the dialog reports "Event <short-id> is not a NIP-34 patch (kind <k>)." and no review is started. (The fetch uses an `ids`-only filter so non-1617 events are returned and rejected here, rather than filtered out upstream as "not found".)
...
- [ ] **PR with no changes rejected** `AC-srm-pr-open-empty-diff`: Given a PR's tip commit and merge-base resolve to the same tree (no net changes), when the app computes the diff, then it reports "Pull request <short-id> has no changes." and no review is started.
...
5. **Pre-existing NIP-34 errors in the shared spec and CLI**: The shared `product/shepherd-review.md` (`FR-sr-patch-fetch`, `FR-sr-patch-validation`, `FR-sr-patch-metadata-display`) and the `/shepherd-review` command prompt (`.claude/commands/shepherd-review.md`) currently state that NIP-34 patch events are kind `1617` ("proposal") or `1621` ("patch"), and that patch status is read from a `status` tag on the patch event. Per NIP-34 both are wrong: kind `1617` is **patch** (there is no "proposal" kind), kind `1621` is an **issue** (not a patch), and patch status is conveyed by separate kind `1630`–`1633` status events, not a tag on the patch event. The in-app path (`FR-srm-patch-open-fetch`, `FR-srm-patch-open-load`) is written NIP-34-correctly and does not inherit the shared spec's kind set or status-tag claim. Correcting the shared spec and CLI command file is a separate follow-up because it changes CLI behavior (the command's `nak req -k 1617 -k 1621` filter and its `status`-tag read would both change); it is out of scope for the in-app patch-open task.
