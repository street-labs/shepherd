...
After the server is running, the command opens the CRPG in a standalone application window. The launch URL includes parameters that tell the app which file to load and which session to target (e.g., `http://localhost:<port>?session=<id>&file=<encoded-path>`). Standalone windows look and feel like a dedicated application. The command attempts to open in a standalone application window (platform mechanism varies); if a standalone window cannot be opened, it falls back to the default browser. Each session ID opens its own application window. Invoking a new session from a different worktree does NOT replace an existing session's window. If a session is re-invoked with the same session ID, the existing window may be reused (this is the within-session reload case, e.g., running `/shepherd` again from the same worktree).
...
**Given** a supported standalone window mechanism is available, **when** the user runs `/shepherd <filepath>`, **then** the CRPG opens in a standalone application window (no address bar, no tabs, no navigation chrome). If a standalone window cannot be opened, the CRPG falls back to opening in the default browser as a regular tab.
...
**Given** the slash command is installed on macOS, Linux, or Windows, **when** the user runs `/shepherd <filepath>`, **then** the CRPG opens in an app-mode window using the platform-appropriate mechanism. Falls back to the default browser if a standalone window is not available.
