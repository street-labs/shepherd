...
## 2026-06-18 -- iOS PR review via referenced patch events (iterate patches), not git
**Context**: PR #41 added NIP-34 kind `1618` PR review as macOS-only for v1, because the macOS path acquires a PR's diff by shelling out to `git` (fetch the `clone` URL, `git diff <merge-base>..<c>`), and iOS has no git binary and invokes no subprocess (`NFR-sri-no-git`). Luke asked for iOS support and suggested "iterate the patches." A NIP-34 PR event may carry `e` tags pointing at kind `1617` patch events (NIP-34 documents a PR as a revision of, or assembling, existing patches), and a kind `1617` patch's content is an inline unified diff — exactly what iOS already reviews with zero git.
**Decision**: iOS reviews a kind `1618` PR by fetching the PR event in-process, reading its `e` tags that reference kind `1617` patch events, fetching each referenced patch over the relay client, splitting and unioning their inline diffs into one changeset, and attaching PR metadata with the PR event as the thread root. No git, no subprocess, no new dependency — it reuses 100% of the existing iOS patch infrastructure. The Open Patch affordance becomes "Open Patch or PR" and dispatches on event kind (`1617` → patch load, `1618` → PR load). A git-only PR whose changes were never published as patch events (no valid `e`-tagged kind `1617` references) is rejected with "PR <short-id> has no reviewable patch events. Its changes may be available only via git clone — open this PR on macOS." This graceful-degradation case is the documented ceiling, not a bug.
**Alternatives considered**: (a) Remote diff endpoint — if the PR's `clone` URL is an HTTPS grasp/ngit server serving `diff <base>..<tip>` as plain text, iOS fetches one URL and gets the unified diff without a git client. Covers git-only PRs too, but depends on server support not verified to exist in the grasp ecosystem; deferred to a roadmap follow-up. (b) Ship macOS-only and defer iOS PRs — rejected per Luke's request for iOS support. (c) Bundle a git library on iOS — rejected as heavy and against `NFR-sri-no-git`'s no-subprocess posture.
**Rationale**: Iterate-patches is the lazy path: it ships now using only the relay client and the existing splitter, covers PRs whose authors published the changes as patch events, and degrades clearly for git-only PRs. No new dependency, no subprocess, no server-side assumption.
**Consequences**: New iOS slugs `FR-sri-pr-open-patches`, `FR-sri-pr-open-load` and ACs (`AC-sri-pr-open-happy`, `AC-sri-pr-open-no-patches`, `AC-sri-pr-open-patch-not-found`, `AC-sri-pr-open-patch-wrong-kind`, `AC-sri-pr-open-metadata`, `AC-sri-pr-open-activates-thread`). `FR-sri-patch-open-fetch` kind contract now accepts `1618` and dispatches; `FR-sr-pr-diff-acquisition` does not apply on iOS (realized by `FR-sri-pr-open-patches` instead); `NFR-sri-no-git` reframed to state PRs are reviewed via referenced patches, with git-only PRs as the ceiling. Open Question 5 updated. Index, glossary, iOS engineering + QA specs updated. `OpenPatchFeature` gains a PR branch that fetches `e`-tagged patch ids, validates/splits each, and unions the diff blocks. Git-only-PR support via a remote diff endpoint is a roadmap follow-up.
**Slug references**: `FR-sri-pr-open-patches`, `FR-sri-pr-open-load`, `FR-sri-patch-open-fetch`, `NFR-sri-no-git`, `FR-sr-pr-source`, `FR-sr-pr-diff-acquisition`, `FR-sr-pr-metadata-display`
