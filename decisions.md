...
**Consequences**: Two new Swift files (`BunkerClient.swift`, `NIP04Crypto.swift`) and modifications to `IdentityClient`, `NostrSigner` (async), `ReviewerIdentity` (source kind + connection state), `IdentityIndicatorView` (bunker states), and `AppFeature` (connect lifecycle + bunker-failure/retry). No new Swift package dependency — NIP-04 uses `P256K` (already a dependency) for ECDH and `CryptoKit` for AES-256-CBC. The local-key path is unchanged. New slugs: `FR-sr-bunker-signing`, `FR-srm-bunker-connect`, `FR-srm-bunker-sign-failure` (and matching ACs).
