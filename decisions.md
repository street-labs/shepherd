...
## 2026-06-28 — Commit-scoped review modes for `/shepherd-mac-review` (macOS only)
**Context**: `/shepherd-mac-review` only reviewed the working copy (uncommitted changes). Developers wanting to review a whole branch against `main`, a single commit, or a range of commits had no way to do so, and an empty working tree could surface as a confusing blank/stale native window. The shared product spec's `FR-sr-changeset-detection` nominally described branch-vs-main as the default, but the implemented command had drifted to working-tree-vs-`HEAD`.
**Decision**: Add three commit-scoped modes to the macOS variant only — `--branch [base]` (merge-base three-dot vs base, default `main`), `--commit [ref]` (single commit vs its parent, default `HEAD`), and `--range <range>` (git range) — alongside the existing working-tree scopes. Keep the macOS default as working-tree-vs-`HEAD` (matching real day-to-day use), and add a hard empty-changeset guard so the app never launches with nothing to review. These macOS requirements supersede the shared `FR-sr-changeset-detection` / `FR-sr-scope-argument` for `/shepherd-mac-review`.
**Alternatives considered**: (a) Overload the existing bare `<ref>` arg to mean "branch vs ref" — rejected because `<ref>` already means working-tree-vs-ref and changing it would be a silent breaking change. (b) Change the default to branch-vs-main to match the shared spec — rejected because the everyday case is reviewing uncommitted work, and a vs-main default would surprise users. (c) Apply the new modes to the web variant too — deferred; the user scoped this to macOS, and the macOS supplement is the correct home so downstream drift-detection hashes (which track the shared spec) stay valid.
**Rationale**: Explicit flags keep each scope unambiguous and back-compatible. Putting the new requirements in `product/macos/shepherd-review.md` (not the shared spec) means the macOS design/engineering/QA specs' `product-hash`/`product-slugs` frontmatter — stamped against the shared spec — needs no recompute. The empty-changeset guard is the deterministic fix for the blank-window symptom: the command refuses to invoke the launcher when zero reviewable files remain and prints a scope-specific message instead.
**Consequences**: `.claude/commands/shepherd-mac-review.md` and its opencode mirror gained scope parsing, per-mode git command mapping (commit scopes exclude untracked files), and the empty guard. No Swift or launcher change. A separate operational note: editing Swift sources without re-running `./scripts/install-command.sh` leaves a stale prebuilt binary — an independent cause of "the app looks wrong" addressed by rebuilding, not by this command.
**Slug references**: `FR-srm-scope-modes`, `FR-srm-branch-scope`, `FR-srm-commit-scope`, `FR-srm-range-scope`, `FR-srm-commit-mode-no-untracked`, `FR-srm-no-blank-window`, `AC-srm-default-scope`, `AC-srm-branch-scope`, `AC-srm-commit-scope`, `AC-srm-range-scope`, `AC-srm-commit-excludes-untracked`, `AC-srm-empty-no-launch`

## 2026-06-29 — Remove the web platform; Shepherd is macOS-only
**Context**: Shepherd shipped two platforms (`web`, `macos`) for the Code Review Prompt Generator. The team decided to go all-in on the native macOS app and drop the browser-based web variant entirely.
**Decision**: Delete the web platform — `engineering/apps/web/` (React/Vite app), the web design/engineering/QA specs (`design/web/`, `engineering/web/`, `qa/web/`, `product/web/`), the web launcher (`scripts/shepherd-launch.sh`), the web slash-command files and opencode skills (`/shepherd`, `/shepherd-review`), and the `web` entry in `pdeq.json`. The platform-neutral *product* specs for web-only features (`dark-mode`, `diff-view`, `markdown-render`) are kept (the macOS app realizes these natively); only their web downstream specs were removed. Repointed `justfile`/`run-tests.sh` to the Swift app and scrubbed `index.md`, `glossary.md`, `CLAUDE.md`, and `README.md` of web references.
**Alternatives considered**: (a) Keep both platforms — rejected; the team is committing to native macOS and the web app was maintenance overhead. (b) Also delete the orphaned product specs for dark-mode/diff-view/markdown-render — rejected for now to preserve product intent; they can be re-specced for macOS or retired later.
**Rationale**: A single platform removes the dual-stack maintenance burden (two test suites, two launchers, the Vite/server/browser handoff machinery) and lets the spec tree and slash commands collapse toward a single canonical `/shepherd` + `/shepherd-review` (handled in the follow-up rename). Keeping `macos/` subfolders (rather than flattening to single-platform) minimizes churn and leaves all drift-detection hashes valid.
...
## 2026-07-23 — NIP-46 bunker as a second reviewer-identity form (no raw nsec required)
**Context**: The reviewer identity (`FR-srm-identity-load`) currently requires a raw `nsec` on the host (env var or config file), which is poor key custody for a review machine. The user asked to support "bunker logins" so a raw secret key never has to be entered.
**Decision**: Add a NIP-46 bunker connection as a second identity form alongside the existing local secret key. The reviewer configures a `bunker://<remote-signer-pubkey>?relay=<wss-url>[&secret=<token>]` URI (via `SHEPHERD_BUNKER` / `~/.config/nostr/bunker`); the app runs the NIP-46 `connect` handshake, obtains the reviewer's (user) pubkey via `get_public_key`, and delegates each `sign_event` to the bunker. The bunker sources are checked **before** the nsec sources so the bunker form is preferred and a raw key need not live on the host. A malformed `bunker://` URI is a distinct parse-error state (not silently no-identity). Signing is unified behind one async `sign(event:)` interface that dispatches to in-process Schnorr (local key) or `sign_event` (bunker), so the publish path is form-agnostic. If the bunker cannot sign, the comment is retained locally with a bunker-named error and the reviewer can retry — never silently dropped.
**Alternatives considered**: (a) Always require a local key and shell out to a separate signer process — rejected, reintroduces the raw-key-on-host problem and a process-boundary key exposure. (b) Add a full Nostr SDK dependency for NIP-46 — rejected; NIP-46 needs only ECDH + NIP-44 (ChaCha20-Poly1305 + HKDF) over the existing `RelayClient` WebSocket transport, both already available via `P256K` + `CryptoKit`. (c) Use NIP-04 + AES-256-CBC for the kind `24133` payloads as an earlier draft did — rejected after review; the current NIP-46 encrypts kind `24133` with NIP-44 (not NIP-04), and CryptoKit exposes no AES-CBC API, so a NIP-04/AES-CBC client would neither interoperate with current bunkers nor build without CommonCrypto. NIP-44 fixes both. (d) Make signing synchronous and block the publish path — rejected; a bunker round-trip is a network call, so the signing interface is async.
**Rationale**: NIP-46 is the standard Nostr remote-signing protocol and `nak bunker` / nsecbunker are established tools, so the reviewer can bring an existing bunker. Reusing the in-process `RelayClient` (already used for patch-thread subscriptions) keeps the transport self-contained with no new WebSocket code. The unified async signer keeps the rest of the publish path unchanged.
**Consequences**: Two new Swift files (`BunkerClient.swift`, `NIP44Crypto.swift`) and modifications to `IdentityClient`, `NostrSigner` (async), `ReviewerIdentity` (source kind + connection state), `IdentityIndicatorView` (bunker states), and `AppFeature` (connect lifecycle + bunker-failure/retry). No new Swift package dependency — NIP-44 uses `P256K` (already a dependency) for ECDH and `CryptoKit` (`ChaChaPoly`, `HKDF`) for ChaCha20-Poly1305 + HKDF; no AES-CBC. The `connect` request carries the bunker (remote-signer) pubkey as `params[0]` (not the session pubkey) plus client metadata, per the current NIP-46. The local-key path is unchanged. New slugs: `FR-sr-bunker-signing`, `FR-srm-bunker-connect`, `FR-srm-bunker-sign-failure` (and matching ACs).
**Slug references**: `FR-srm-identity-load`, `FR-srm-event-sign`, `FR-srm-bunker-connect`, `FR-srm-bunker-sign-failure`, `FR-srm-identity-indicator`, `FR-sr-reviewer-identity`, `FR-sr-bunker-signing`

## 2026-07-25 — Add iOS as a platform (patch review + commenting on iPhone and iPad)
**Context**: Shepherd is macOS-only today. The owner wants an iOS app that can load any ngit (NIP-34) patch and let the reviewer interact with comments, on both iPhone and iPad. Local file loading, the slash-command mode, and the local-server handoff do not map to iOS, but ngit patch review + the bidirectional patch-thread commenting loop do.
**Decision**: Add `ios` as a platform (`pdeq.json`, `project.md`) scoped to patch review and commenting only. Port the macOS in-app patch open and the patch-thread bidirectional publishing loop to iOS (new slugs `FR-sri-*`, `AC-sri-*`), and port the CRPG review surface as iOS supplements (`FR-crp-ios-*`) that scope out local file loading (`FR-crp-file-load`), the Done/server handoff (`FR-crp-done-action`, `FR-crp-prompt-handoff`), slash-command session routing (`FR-sc-*`), and panel resizing (`FR-crp-panel-resize`), replacing them with an adaptive iPhone/iPad layout, system-appearance following, clipboard copy, and background-resume preservation. The iOS entry point is Open Patch only; content comes solely from an in-app fetched NIP-34 patch (kind `1617`, NIP-34-correct — issues/PRs out of scope v1). Reviewer identity is configured in-app (local key or NIP-46 bunker) since iOS has no env vars/dotfiles. No new top-level product spec was created — this is a new presentation of existing features (`code-review-prompt` + `shepherd-review`), per structural triage.
**Alternatives considered**: (a) Create a new top-level `ios-patch-review` product spec — rejected; the behavior is already specified in the shared `code-review-prompt` and `shepherd-review` specs, so a new top-level spec would duplicate and risk the structural placement error. (b) Include local file paste/upload on iOS — rejected by the owner; local files don't make sense on iOS and the patch-open entry is the value. (c) Share the macOS Nostr dependencies (`RelayClient`/`NostrSigner`/`Bech32`/`PatchReplyMapper`/`NostrEvent`) by lifting them into a shared multiplatform SPM target in this kickoff — rejected as scope; that refactor touches the macOS app and is deferred to a follow-up. For v1 the iOS app target carries its own copies under `engineering/apps/ios/Sources/Dependencies/`. (d) Mark reused-macOS-module Code Map rows as `implemented` — corrected to `planned`, since the iOS app does not exist yet; the modules are reused but the iOS build/wiring is pending.
**Rationale**: iOS is a natural second surface for Shepherd's patch-review loop: reviewers can triage and comment on ngit patches away from a machine. Scoping to patch review keeps the iOS app coherent (one entry point) and avoids porting macOS-only machinery (CLI, git, server) that has no iOS analog. Reusing the cross-platform SPM feature modules (SwiftUI + TCA) avoids forking the review logic. The NIP-34-correct kind/diff handling matches the macOS in-app path's existing correction.
**Consequences**: 8 new spec files: `product/ios/{code-review-prompt,shepherd-review}.md`, `design/ios/{…}.md`, `engineering/ios/{…}.md`, `qa/ios/{…}.md`; new slugs `FR-crp-ios-*`, `NFR-crp-ios-*`, `AC-crp-ios-*`, `FR-sri-*`, `NFR-sri-*`, `AC-sri-*`; `index.md` updated (new slug entries + iOS reference lines on shared slugs); `pdeq.json` and `project.md` platforms updated. No code written (kickoff is specs-only). Implementation open questions: lift Nostr deps to a shared multiplatform target; verify feature modules compile for iOS; TreeSitter grammar-bundle packaging on iOS; roster/display-name resolution on iOS (v1 = truncated npub); identity persistence via Keychain (v1 = in-memory).
**Slug references**: `FR-crp-ios-patch-only-entry`, `FR-crp-ios-adaptive-layout`, `FR-crp-ios-system-appearance`, `FR-crp-ios-clipboard`, `FR-crp-ios-background-handoff`, `NFR-crp-ios-min-version`, `NFR-crp-ios-launch-time`, `NFR-crp-ios-memory`, `FR-sri-patch-open-entry`, `FR-sri-patch-open-input`, `FR-sri-patch-open-fetch`, `FR-sri-patch-open-load`, `FR-sri-identity-load`, `FR-sri-bunker-connect`, `FR-sri-event-sign`, `FR-sri-bunker-sign-failure`, `FR-sri-event-publish`, `FR-sri-comment-publish-on-submit`, `FR-sri-reply-to-reply`, `FR-sri-identity-indicator`, `NFR-sri-no-server`, `NFR-sri-no-git`, `NFR-sri-platform-restriction`, `FR-sr-relay-client`, `FR-sr-patch-replies-display`, `FR-sr-patch-replies-live`, `FR-sr-patch-metadata-display`, `FR-sr-patch-reply-publish`, `FR-sr-reviewer-identity`, `FR-sr-bunker-signing`

## 2026-07-25 — Port the Identity feature (in-app nsec + bunker login) to iOS
**Context**: The iOS platform kickoff scoped identity into the shepherd-review supplement (`FR-sri-identity-load` etc.), covering the publishing half. But the in-app identity screen itself — login with an existing `nsec`, create-a-new-identity, login with a NIP-46 `bunker://` URI, persistence, logout, backup reveal — is its own shared feature (`product/identity.md`). Luke flagged that identity should be included on iOS, with a way to log in via nsec and bunker. On iOS this screen is especially important: there are no environment variables or dotfiles, so the in-app screen is the only identity configuration path.
**Decision**: Port the shared Identity feature to iOS as a new presentation of an existing feature (no new top-level product spec). Add `product/ios/identity.md` (supplement), `design/ios/identity.md`, `engineering/ios/identity.md`, `qa/ios/identity.md`. The shared requirements apply as-is except `FR-id-out-of-band-honored`, `FR-id-no-silent-override`, and `AC-id-out-of-band-skips`, which do not apply on iOS (no out-of-band sources exist). Two iOS-specific slugs capture the platform divergence: `FR-id-ios-keychain-storage` (persistence via the iOS Keychain) and `FR-id-ios-screen-is-only-path` (the in-app screen is the sole identity path). The identity screen is a SwiftUI sheet/full-screen form over the empty state (iOS has no multi-window), not a separate window as on macOS. Keychain is the secure store; the load precedence is Keychain-only. The produced identity flows into the iOS publishing path (`FR-sri-event-sign`, `FR-sri-bunker-connect`, `FR-sri-identity-indicator`) already specified.
**Alternatives considered**: (a) Leave identity folded into the shepherd-review iOS supplement — rejected; the in-app login/create screen is a distinct shared feature with its own slug set, and folding it leaves nsec-login/create-new/backup-reveal/logout unspecified on iOS. (b) Create a new top-level `ios-identity` product spec — rejected; the behavior is already in `product/identity.md`, so this is a new presentation, not a new feature (structural triage). (c) Reuse the macOS KeychainClient/IdentityClient directly — deferred; per the existing iOS kickoff decision, Nostr/identity deps are duplicated into the iOS target for v1 and lifted to a shared multiplatform target as a follow-up.
**Rationale**: iOS reviewers need the same self-service identity setup as macOS, and it is the only path on a platform without env/config files. Porting the shared spec keeps the behavior consistent across platforms while recording the iOS-specific divergences (Keychain-only, no out-of-band, sheet-not-window).
**Consequences**: 4 new spec files under `product/ios`, `design/ios`, `engineering/ios`, `qa/ios` (identity.md). New slugs: `FR-id-ios-keychain-storage`, `FR-id-ios-screen-is-only-path`, `AC-id-ios-keychain-storage`, `AC-id-ios-screen-is-only-path`. `index.md` updated (4 new entries + iOS reference lines on the 36 shared identity slugs). No code written (specs-only). Implementation open questions: lift identity/Nostr deps to a shared multiplatform target; verify `swift-secp256k1` builds for iOS; Keychain access-group vs default; sheet vs full-screen cover for first-run.
**Slug references**: `FR-id-nsec-login`, `FR-id-create-new`, `FR-id-show-new-nsec`, `FR-id-persistence`, `FR-id-active-indicator`, `FR-id-logout`, `FR-id-bunker-login`, `FR-id-bunker-persist`, `FR-id-bunker-connect-failure`, `FR-id-screen-when-no-identity`, `FR-id-optional-reentry`, `NFR-id-no-plaintext-key`, `NFR-id-key-stays-local`, `NFR-id-login-latency`, `NFR-id-bunker-connect-latency`, `NFR-id-key-validity`, `FR-id-ios-keychain-storage`, `FR-id-ios-screen-is-only-path`, `AC-id-ios-keychain-storage`, `AC-id-ios-screen-is-only-path`, `FR-sri-event-sign`, `FR-sri-bunker-connect`, `FR-sri-identity-indicator`

## 2026-07-07 -- markdown-render scoped to file-mode; rendered diff deferred to roadmap
**Context**: Conformance audit (SH-tqwqtfkh) found markdown-render fully spec'd (12 FR) with zero code, no macOS design/eng/QA specs, a web-shaped product spec (HTML/XSS/ARIA), and 4 rendered-diff FRs conceptually dependent on the also-unimplemented diff-view.
**Decision**: Build file-mode only. Retain 8 file-mode FRs (detect, toggle, render commonmark/GFM, styling, element-id, rendered-comment-create, rendered-comment-prompt, switch-comments) + 5 NFRs + 14 ACs. Move the 4 rendered-diff FRs (FR-mdr-raw-diff-unchanged, -rendered-diff-display, -rendered-diff-comment, -rendered-diff-prompt), NFR-mdr-rendered-diff-perf, and 6 diff ACs to roadmap/markdown-render.md. Reconcile product spec to platform-neutral language (kept permanent slugs NFR-mdr-xss-safety, AC-mdr-html-sanitized; reworded bodies to "neutralize active content" since native AttributedString rendering has no web/JS execution surface).
**Alternatives considered**: Descope entirely to roadmap (rejected — user wants the file-mode value now); build all 12 including rendered diff (rejected — blocked on diff-view, ~week+ effort).
**Consequences**: 11 diff slugs removed from index.md and product spec; roadmap/markdown-render.md now tracks the deferred V2. New design/eng/QA macOS specs created for the 8 FRs.
## 2026-07-07 -- Rendered markdown parser: apple/swift-markdown; separate renderedComments store
**Context**: The rendered view needs a semantic block/inline AST with source-line ranges for element IDs (FR-mdr-element-id) and raw-source prompt mapping (FR-mdr-rendered-comment-prompt), plus native GFM table/strikethrough/task-list support. The existing comment model is line-anchored (Comment.startLine/endLine, single flat allComments store); rendered mode breaks the 1:1 line mapping.
**Decision**: Add apple/swift-markdown (Markdown product) to SharedModels for parsing. Keep pure model (MarkdownDocumentModel → [MarkdownBlock] with deterministic positional element IDs + source ranges) and MarkdownPromptBuilder in SharedModels for value-snapshot testing; SwiftUI rendering (MarkupVisitor) in CodeViewerFeature. Introduce a separate renderedComments: IdentifiedArrayOf<RenderedComment> store keyed by element ID, leaving allComments and the raw view byte-identical (AC-mdr-raw-unchanged). Render mode is per-file UI state (renderModeByFile), default raw.
**Alternatives considered**: Roll a renderer on the already-present tree-sitter-markdown grammar (rejected — no semantic block/inline convenience, painful GFM tables); Foundation AttributedString(markdown:) (rejected — inline-only, no block structure); extend Comment with an element anchor / unified store (rejected — decisions.md precedent favors separate per-mode stores; keeps raw view untouched).
**Consequences**: New SharedModels dependency on swift-markdown (vendors cmark-gfm). tree-sitter-markdown remains the raw-view highlighter; the two coexist. renderedComments is a third comment concept alongside the historical file/diff split, but only file + rendered exist in code today.

## 2026-07-28 — Deeplink entry: patches-only v1, PRs and iOS roadmaped; reuse in-app open-patch load path
**Context**: Luke wants deeplink abilities so Buzz agents can send him a link that opens an ngit patch (or PR) directly in Shepherd. The `shepherd://` URL scheme is already registered in `Info.plist` and `.handlesExternalEvents` already dedupes to the existing window, but no URL payload is parsed or routed. The in-app Open Patch path (`FR-srm-patch-open-*`) already loads kind `1617` patches in-app; PRs (kind `1618`) are explicitly out of scope for v1 in-app review (they are addressable entities fetched by clone URL + commit, not an inline diff, and need their own in-app review surface).
**Decision**: Kickoff scoped to macOS deeplinks for **patches (kind `1617`) only** in v1. A patch deeplink (`shepherd://patch/<ref>`, hex id or `nevent1`) reuses the existing in-app open-patch fetch-validate-load path via a new shared `PatchFetcher` helper (extracted from `OpenPatchFeature`), so one fetch-validate implementation serves both the dialog and the deeplink. New `AppFeature` handling covers cold launch (straight to loading), warm launch into the empty state (load), and warm launch with a review in progress (confirm before replacing; no-comments case skips confirm). Errors surface via the existing `AlertState` notice path with the same per-cause wording as the Open Patch dialog; malformed/unsupported links are rejected cleanly without disturbing an in-progress review. PRs (kind `1618`) and issues (kind `1621`) deeplinks, and iOS deeplinks, are deferred to the roadmap — they depend on in-app PR/issue review and an iOS URL-scheme/universal-link registration that do not exist yet.
**Alternatives considered**: (a) Include PRs in v1 — rejected; PRs need a new in-app review surface (clone URL + commit, addressable `naddr` fetch), which is a separate, larger feature, not a URL handler. Wiring a `shepherd://pr/<ref>` route to nothing would ship a broken link. (b) Present the Open Patch dialog pre-filled from the deeplink — rejected; the reviewer clicked a link, so presenting a dialog they didn't ask for and making them press Fetch is extra friction; the link is the request. (c) Duplicate the fetch-validate sequence in a deeplink-only path — rejected; extracting `PatchFetcher` keeps one implementation behind both entry points and means the deeplink inherits dialog-correct error behavior for free. (d) Queue a second deeplink arriving while the first is still fetching — rejected for v1; the rare edge case is handled with a `.alreadyLoading` notice rather than a queue, to avoid interleaving two fetches without adding queueing machinery.
**Rationale**: Patches are the cheap, high-value half and reuse an already-built in-app load path; the only new work is URL parsing, a shared fetch helper, and launch-state/error handling. Parking PRs/iOS is honest about scope and avoids shipping a PR deeplink route that cannot load. Reusing the existing `patchLoaded` load path and `AlertState` notice surface means no new render surface and no new identity/context path.
**Consequences**: New macOS-specific slugs `FR-srm-deeplink-*` (scheme, patch-format, route, cold-launch, warm-empty, warm-in-progress, malformed, errors), `NFR-srm-deeplink-latency`, and matching `AC-srm-deeplink-*`. New shared `PatchFetcher` dependency (extracted from `OpenPatchFeature`, which is rewritten to call it — no dialog behavior change). New `AppFeature` deeplink state/actions and an `.onOpenURL` handler in `ShepherdApp.swift`; `Info.plist` and `.handlesExternalEvents` already present. The shared `product/shepherd-review.md` is unchanged (deeplink is platform-specific behavior → lives in the macOS supplement), so downstream `product-hash`/`product-slugs` frontmatter stays valid. Roadmap gains PR/issue deeplink and iOS deeplink fast-follows.
**Slug references**: `FR-srm-deeplink-scheme`, `FR-srm-deeplink-patch-format`, `FR-srm-deeplink-route`, `FR-srm-deeplink-cold-launch`, `FR-srm-deeplink-warm-empty`, `FR-srm-deeplink-warm-in-progress`, `FR-srm-deeplink-malformed`, `FR-srm-deeplink-errors`, `NFR-srm-deeplink-latency`, `FR-srm-patch-open-fetch`, `FR-srm-patch-open-load`

## 2026-07-28 — Widen deeplink kickoff to include pull requests (kind 1618)
**Context**: The initial deeplink kickoff (PR #43) scoped v1 to patches (kind 1617) only, parking PRs as a roadmap fast-follow on the grounds that PRs "are addressable NIP-34 entities needing their own in-app review surface." On reviewing the NIP-34 spec, that assessment was partly wrong: kind 1618 PRs are NOT addressable (not parameterized kind 3xxxx) — they are regular events referenced by event id or `nevent1`, same as patches. The real difference is the content: a patch carries an inline diff; a PR carries a markdown description plus `clone`/`c`/`merge-base` tags pointing to a git repo. Luke asked to include PRs.
**Decision**: Widen the kickoff to include pull requests (kind 1618). Add an in-app PR open path (`FR-srm-pr-open-*`) that fetches the kind-1618 event, clones the git repository at the `clone` URL, computes the unified diff (`merge-base..tip` when `merge-base` is present, otherwise the tip commit against its parent), and feeds it through the same `PatchDiffSplitter` → diff-as-tabs review surface as patches. Add a `shepherd://pr/<ref>` deeplink action that routes through the same fetch-clone-diff-load path. The Open Patch dialog is extended to accept both kinds — the app fetches by event id and branches on the fetched kind, so the reviewer does not specify patch-vs-PR. A new `GitClient` dependency (shelling out to `/usr/bin/git` via `Process`) handles the clone and diff; the in-app patch path's no-git guarantee is preserved (only PR review requires git, per `NFR-srm-pr-open-git-required`). PR metadata (`PRMetadata`) mirrors `PatchMetadata` with PR-specific fields (subject, tip commit, clone URL, branch name, merge-base). The live reply subscription and publish path key off the event id regardless of kind — the NIP-22 thread mechanism is identical for patches and PRs. Issues (kind 1621) remain out of scope (no diff, no clone URL — a fundamentally different surface).
**Alternatives considered**: (a) Keep PRs roadmaped — rejected; Luke asked to include them and the PR path reuses most of the patch infrastructure (PatchDiffSplitter, review surface, thread, publishing). (b) Use a Swift git library (SwiftGit2/libgit2) instead of shelling out — rejected; `/usr/bin/git` is already a system dependency for the CLI path, `Process` is stdlib, and a git library is a large new dependency for a clone+diff operation. (c) Fetch diffs from a hosting platform API (GitHub/GitLab) — rejected; NIP-34 `clone` URLs are generic git URLs, not platform-specific. (d) Separate "Open PR" dialog/button — rejected; the app fetches by event id and branches on kind, so one dialog/field/button serves both; adding a separate affordance is unnecessary UI. (e) Full multi-commit PR diff without `merge-base` — rejected for v1; `git show <tip>` (tip against parent) is the fallback, showing at least the tip changes; full multi-commit without `merge-base` is a fast-follow.
**Rationale**: PRs are the other half of what Luke asked for ("patches and PRs"). The PR load path reuses the patch review surface, thread, and publishing infrastructure — the only new work is the git clone+diff step (GitClient) and PR-specific metadata. Shelling out to git is the cheapest correct approach and doesn't add a Swift dependency. Correcting the "PRs are addressable" error was necessary regardless — the spec had a factual NIP-34 error that would have misled future work.
**Consequences**: New macOS-specific slugs `FR-srm-pr-open-fetch`, `FR-srm-pr-open-clone`, `FR-srm-pr-open-load`, `NFR-srm-pr-open-git-required`, `FR-srm-deeplink-pr-format`, and matching `AC-srm-pr-open-*` / `AC-srm-deeplink-pr-*`. New `GitClient` dependency, `PRMetadata` model, `ReviewContext.prMetadata` field, `PatchDiffSplitter.split(_:)` entry point, and `PatchFetcher.FetchResult.pr` case. `OpenPatchFeature` gains PR/git failure states. The roadmap PR-deeplink fast-follow is removed (now in scope); the issue-deeplink fast-follow remains. The shared `product/shepherd-review.md` remains unchanged (PR review is platform-specific → macOS supplement only), so downstream frontmatter stays valid. PR #43 title and description are updated to reflect the widened scope.
**Slug references**: `FR-srm-pr-open-fetch`, `FR-srm-pr-open-clone`, `FR-srm-pr-open-load`, `NFR-srm-pr-open-git-required`, `FR-srm-deeplink-pr-format`, `FR-srm-deeplink-route`, `FR-srm-patch-open-fetch`, `FR-srm-patch-open-load`
## 2026-06-18 -- Add NIP-34 pull request (kind 1618) review support, macOS-only for v1
**Context**: Shepherd already reviews NIP-34 patches (kind `1617`), whose event content is the diff. NIP-34 pull requests (kind `1618`) are a distinct contribution type: a PR event's content is a markdown description and its tags point at the proposed changes by reference (`clone` URL, `c` tip commit, optional `merge-base` and `branch-name`). Reviewing a PR therefore requires fetching the referenced git objects and computing the diff — the diff is not in the event. The previous spec explicitly excluded kind `1618` from in-app review ("out of scope for v1").
**Decision**: Add PR review support as a new changeset source alongside patches. CLI path: `/shepherd-review --pr <event-id>`. In-app path: the Open Patch dialog is extended to "Open Patch or PR…" and dispatches on event kind (`1617` → patch load, `1618` → PR load). The PR diff is acquired by shelling out to `git` (fetch the clone URL's tip + merge-base, `git diff <merge-base>..<c>`); the resulting diff flows through the existing split/filter/review pipeline and the patch-thread reply loop activates with the PR event as thread root. PR review is **macOS-only for v1** — iOS has no git and invokes no shell process (`NFR-sri-no-git`), so PRs are rejected on iOS with a clear message until a remote-diff-service path exists.
**Alternatives considered**: (a) Reuse the patch inline-diff path for PRs — rejected: PRs have no inline diff. (b) Require a full `git clone` of the PR repo — rejected as too heavy; fetch only the needed commits. (c) Support PRs without a `merge-base` tag by diffing the tip against its first parent or the base repo's HEAD — rejected for v1; first-parent diffs misrepresent a multi-commit PR, and resolving the base repo's HEAD requires fetching the kind `30617` repo announcement, which is scope creep. v1 requires a `merge-base` tag and rejects PRs without one with a clear error. (d) Defer in-app PR open and ship CLI-only — rejected: "the app" supporting PRs means the in-app entry too.
**Rationale**: PRs are a core NIP-34 contribution type alongside patches; supporting them makes Shepherd the review tool for both Nostr submission kinds. Shelling out to `git` is the cheapest correct acquisition path on macOS (git ships with the Xcode Command Line Tools). Requiring `merge-base` keeps the diff computation unambiguous and minimal.
**Consequences**: New shared slugs `FR-sr-pr-source`, `FR-sr-pr-fetch`, `FR-sr-pr-diff-acquisition`, `FR-sr-pr-metadata-display` and ACs; new macOS slugs `FR-srm-pr-open-fetch`, `FR-srm-pr-open-diff`, `FR-srm-pr-open-load`, `NFR-srm-pr-git-required` and ACs. New `GitDiffClient` dependency (git subprocess) and `OpenPatchFeature` kind dispatch; `ReviewContext.PatchMetadata` gains optional `tipCommit` and `branchName` fields; `PatchMetadataSectionView` renders PR-specific labels. iOS PR support is a roadmap follow-up (needs a remote diff service, not a local git subprocess). PRs without `merge-base` are rejected in v1 (roadmap follow-up). PR-thread replies reuse the existing kind:1 root-`e`-tag format; aligning to NIP-22 `E`/`P` tagged comments is a shared follow-up with patches.
**Slug references**: `FR-sr-pr-source`, `FR-sr-pr-fetch`, `FR-sr-pr-diff-acquisition`, `FR-sr-pr-metadata-display`, `FR-srm-pr-open-fetch`, `FR-srm-pr-open-diff`, `FR-srm-pr-open-load`, `NFR-srm-pr-git-required`, `NFR-sri-no-git`
...
## 2026-07-23 — In-app nsec login + key creation UI (Identity feature)
**Context**: The reviewer identity (`FR-srm-identity-load`) is currently configured out of band — env vars or `~/.config/nostr/` config files — with no in-app way to set, view, or change it. This is the single biggest friction point between installing Shepherd and publishing review replies, and excludes reviewers who are not already Nostr power users. The user asked for UI to log in with an existing `nsec` and to create a new one if they don't have one.
**Decision**: Add a new Identity feature with an in-app login/create window. A reviewer pastes an existing `nsec1...` to log in, or generates a brand-new identity with one click; the adopted key is persisted in the macOS Keychain (never plaintext on disk) and reused across launches. The existing out-of-band identity-load path is preserved and extended: a Keychain source is added as the **highest** precedence (above `SHEPHERD_BUNKER`, `~/.config/nostr/bunker`, `SHEPHERD_NSEC`, `~/.config/nostr/identity`), so an in-app login is the reviewer's most recent explicit action and is not silently overridden by a stale env var. An out-of-band identity, when no in-app key is stored, is still honored (the screen only appears when no identity is available from any source). The feature reuses the existing `ReviewerIdentity` model, `NostrSigner.derivePublicKey`, `Bech32`, and `swift-secp256k1` (`P256K`) for key generation — no new third-party dependency.
**Alternatives considered**: (a) Write the in-app key to the existing `~/.config/nostr/identity` plaintext file — rejected; `NFR-id-no-plaintext-key` forbids plaintext on-disk storage and Keychain is the native platform facility for secrets (ponytail ladder rung 4). (b) Out-of-band identity wins precedence over in-app — rejected; it would surprise a reviewer who just logged in in-app and expects it to take effect. (c) Prompt the reviewer to choose on conflict — rejected; adds per-launch friction for a rare case. (d) Make the backup-reveal gate require re-pasting the generated key — rejected for v1; a single "I've saved my key" confirmation is the v1 choice (revisit if lost-key reports come in). (e) Add a full Nostr SDK for key generation — rejected; `P256K` already vendored for signing does uniform scalar generation + pubkey derivation.
**Rationale**: The login screen removes the terminal/config-file barrier while keeping the out-of-band path for power users. Keychain is the correct native store for a secret key on macOS and avoids a custom encrypted-file scheme. Highest-precedence in-app storage means the active identity is always the reviewer's most recent explicit choice, satisfying `FR-id-no-silent-override`. Reusing the existing identity model, signing path, and indicator means an in-app-adopted identity flows unchanged into the existing patch-review publish path.
**Consequences**: New `IdentityFeature` TCA module (reducer + view), new `KeychainClient` dependency, and extensions to `IdentityClient` (`loginWithKey`, `createNewIdentity`, `logout`, and the Keychain source in `LoadedIdentity.load`). The bunker path is unaffected (Keychain stores local keys only). New slugs: `FR-id-nsec-login`, `FR-id-create-new`, `FR-id-persistence`, `FR-id-show-new-nsec`, `FR-id-active-indicator`, `FR-id-logout`, `FR-id-out-of-band-honored`, `FR-id-no-silent-override`, `FR-id-screen-when-no-identity`, `FR-id-optional-reentry`, `NFR-id-no-plaintext-key`, `NFR-id-key-stays-local`, `NFR-id-login-latency`, `NFR-id-key-validity` (and matching ACs). Open questions (out-of-band vs in-app precedence resolved per above; login-window placement in launch flow deferred to design; backup-gate strength set to single confirmation for v1).
**Slug references**: `FR-id-nsec-login`, `FR-id-create-new`, `FR-id-persistence`, `FR-id-show-new-nsec`, `FR-id-active-indicator`, `FR-id-logout`, `FR-id-out-of-band-honored`, `FR-id-no-silent-override`, `FR-id-screen-when-no-identity`, `FR-id-optional-reentry`, `NFR-id-no-plaintext-key`, `NFR-id-key-stays-local`, `NFR-id-login-latency`, `NFR-id-key-validity`, `FR-srm-identity-load`, `FR-srm-identity-indicator`, `FR-srm-event-sign`, `FR-sr-reviewer-identity`
