...
## NIP-34 Pull Request
**Definition**: A Nostr event representing a git pull request, per NIP-34 (the Nostr git protocol). A NIP-34 PR is **kind `1618`**; unlike a patch (kind `1617`), its content is a markdown description (not a diff) and its tags point at the proposed changes by reference: a `clone` URL (one or more git clone URLs), a `c` tag (the tip commit of the PR branch), an optional `merge-base` tag (the most recent common ancestor with the target branch), an optional `branch-name` tag, and a `subject` tag (PR title). Reviewing a PR therefore requires fetching the referenced git objects and computing `git diff <merge-base>..<c>`; the diff is not in the event. Shepherd reviews PRs via the CLI (`/shepherd-review --pr <event-id>`) and in-app (the Open Patch or PR dialog routes kind `1618` events to the PR load path, which shells out to `git` to acquire the diff). PR status is conveyed by separate kind `1630`–`1633` status events, same as patches; v1 renders `open`. Reviewers publish kind:1 Patch-Thread Replies against a PR with the PR event as the thread root. On macOS, PR review acquires the diff by shelling out to `git` (fetch the PR's `clone` URL, `git diff merge-base..c`); on iOS, which has no git and invokes no subprocess, PR review acquires the diff from the inline diffs of the kind `1617` patch events the PR references via `e` tags (iterate-patches path) — a git-only PR whose changes were never published as patch events cannot be reviewed on iOS in v1 and is rejected with a pointer to macOS.
**Also known as**: ngit PR, Nostr PR, pull request event
**Not to be confused with**: NIP-34 Patch (kind `1617`, which carries the diff inline in its content, not a clone URL); a GitHub pull request (a vendor-specific PR on GitHub's centralized platform, not a Nostr event); a NIP-34 issue (kind `1621`)
