...
> _Screenshots coming soon._ Generate them on macOS with `./scripts/capture-demos.sh` (renders the app's views to `docs/demos/`), then uncomment the block below.

<!-- Generated by scripts/capture-demos.sh:
![Annotating a file with inline comments](docs/demos/annotate.png)
![Reviewing a branch — the agent's self-review beside each file (/shepherd-review)](docs/demos/shepherd-review.png)
-->
